...
export class UsersService {
  ...
  async create(email: string, password: string): Promise<Users> {
    ...
    if (user) {
      // throw new BadRequestException("Email already claimed!"); Error?How?
    }
    ...
    return this.usersRepo.save(userInstance);
  }
  ...
  async login(email: string, password: string): Promise<Users> {
    ...
    if (!passwordIsCorrect) {
      // throw new UnauthorizedException("Invalid password!"); Throw err in kafka how?
    }
    return user;
  }
